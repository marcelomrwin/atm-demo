...
ARTEMIS_INSTANCE='/home/jboss/amq-broker'
ARTEMIS_DATA_DIR='/opt/artemis-swim/data'
ARTEMIS_ETC_DIR='/home/jboss/amq-broker/etc'
ARTEMIS_OOME_DUMP='/home/jboss/amq-broker/log/oom_dump.hprof'

# The logging config will need an URI
# this will be encoded in case you use spaces or special characters
# on your directory structure
ARTEMIS_INSTANCE_URI='file:/home/jboss/amq-broker/'
ARTEMIS_INSTANCE_ETC_URI='file:/home/jboss/amq-broker/etc/'
...
HAWTIO_ROLE="admin,viewer"

# Java Opts
if [ -z "$JAVA_ARGS" ]; then
    JAVA_ARGS="-XX:AutoBoxCacheMax=20000 -XX:+PrintClassHistogram -XX:+UseG1GC -XX:+UseStringDeduplication  -Dhawtio.disableProxy=true -Dhawtio.realm=console -Dhawtio.offline=true -Dhawtio.rolePrincipalClasses=org.apache.activemq.artemis.spi.core.security.jaas.RolePrincipal -Dhawtio.http.strictTransportSecurity=max-age=31536000;includeSubDomains;preload -Djolokia.policyLocation=${ARTEMIS_INSTANCE_ETC_URI}jolokia-access.xml -Dlog4j2.disableJmx=true -Djava.net.preferIPv4Stack=true -Djava.net.preferIPv4Stack=true  "
fi
...
if [ "$1" = "run" ]; then JAVA_ARGS="$JAVA_ARGS -javaagent:/opt/jboss/container/jolokia/jolokia.jar=config=/opt/jboss/container/jolokia/etc/jolokia.properties"; fi


JAVA_ARGS=" $JAVA_ARGS -Dlog4j2.configurationFile=$ARTEMIS_HOME/conf/log4j2.properties "